...
class Parser:
    def __init__(self, search_string, logger, tags:dict, pipe, processes=2, timeout=0.1):
        ...
        self.session.headers = {'User-Agent': 'Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:81.0) Gecko/20100101 Firefox/81.0', }
    ...
    def get_links_to_parse(self) -> list:
        tags = self.tags
        data = self.session.get('https://zakupki.gov.ru/epz/order/extendedsearch/results.html', params=tags)
        data.raise_for_status()

        links = []
        # получаем количество страниц
        page_num = int(document_fromstring(data.text).xpath('//ul[@class="pages"]/li[last()]/a/span/text()')[0])
        for i in range(1, page_num + 1):
            ...
            links += list(map(lambda x: self.main_link + x, document_fromstring(page_data.text).xpath('//div[@class="registry-entry__header-mid__number"]/a/@href')))
    ...
    def parse_ea44(self, link):
        ...
        tender_id = card_info_container.cssselect('.cardMainInfo__purchaseLink')
        ...
        tender_object = card_info_container.xpath('./div[1]/div[2]/div[1]/span[2]')
        ...
        tender_date = card_info_container.xpath('./div[2]/div[2]/div[1]/div[1]/span[2]')
        if not tender_date:
            tender_date = ''
        else:
        ...
        general_information_container = order_document.xpath('//div[@class="wrapper"]/div[2]/')

        tender_adress = general_information_container.xpath('.//div[@class="col"]/section[3]/span[2]')
        ...
        # условия контракта
        condition_container = self._get_cotract_conditions_container(order_document.xpath('//div[@class="wrapper"]/div[@class="container"]'))
        if condition_container:
            tender_delivery_adress = condition_container.xpath('.//div[@class="col"]/section[2]/span[2]')
            if not tender_delivery_adress:
                tender_delivery_adress = ''
            else:
                tender_delivery_adress = self._normalizer(tender_delivery_adress[0].text_content())
            
            tender_term = condition_container.xpath('.//div[@class="row"]/section[3]/span[2]')
            if not tender_term:
                tender_term = ''
            else:
        ...
        term_document_links = document_fromstring(term_document_data.text).xpath('//span[@class="section__value"]/a[@title]/@href')

        # парсинг результатов определения поставщика
        term_result_link = link.replace('common-info', 'supplier-results')
        term_result_data = self.session.get(term_result_link)
        term_result_data.raise_for_status()
        term_results_doc = document_fromstring(term_result_data.text)
    ...
    def _parse_tender_object_info(self, document):
        ...
        table = document.xpath('//div[@id]/table[@class="blockInfo__table tableBlock"]')
        if not table:
            return json.dumps([])
        else:
            table = table[0]
        ...
        # данные таблицы
        data = [list(map(self._normalizer, row.xpath('./td/text()')))  for row in table.xpath('./tbody/tr[@class="tableBlock__body"]')]

        # создание json документа
        json_data = json.dumps(headers + data)

        return json_data

    
    def _get_cotract_conditions_container(self, containers):
        for element in range(len(containers)):
            name = element.xpath('.//h2').text_content()
            if 'Условия контракта' in self._normalizer(name):
                return element
        return False
...
if __name__ == "__main__":
    ...
    parser = Parser('техно', logger, {'pc': 'on'}, child_pipe)
    process = parser.start_async()
